...
PATH = "E:/Séverin/france IOI,python/finis/anniversaire/anniversaire.csv"
ICONE = "E:/Séverin/france IOI,python/finis/anniversaire/anniversaire.ico"
...
class Accueil(Tk):
    ...
    def actu_meteo(self):
        try:
            sock = urllib.request.urlopen("https://prevision-meteo.ch/services/json/vincennes")
            htmlSource = sock.read()
            sock.close()
            self.meteo = json.loads(htmlSource)
            
            self.affiche_meteo["text"] = f"Température à Vincennes : {self.meteo['current_condition']['tmp']}°C\nCondition : {self.meteo['current_condition']['condition']}"
    ...
    def nouveau(self):
        self.action = True
        showinfo("Nouveautés", """Nouveau (le 02/06) :
- correction d'un bug ne mettant pas la fenêtre en plein écran le jour d'un anniversaire si l'année était définie""")
...
class Modifier(Toplevel):
    ...
    def cherche_fete(self):
        ...
        try:
            ...
            if askyesno("Fête", f"Prénom {prenom} inconnu de la base de donnée. Le signaler ?"):
                r = rq.get("https://smsapi.free-mobile.fr/sendmsg", data={"user":"45325360", "pass":"nQf66Z2TlTsaYq", "msg": "message"})
                if r.status_code != "200":
                    showerror("Erreur", f"Erreur {r.status_code} durant l'envoi.")
                    print(r.text)
                    print(r.headers)
