...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QWidget, QMessageBox, QStyleFactory,
                             QInputDialog, QDialog,
                             QVBoxLayout, QHBoxLayout, QGridLayout,
                             QPushButton, QLabel, QProgressBar, QMenu, QLineEdit,
...
def bissextile(annee=int(strftime("%Y"))):
    if annee % 400 == 0: # 1600, 2000
        return True
    elif annee % 100 == 0: # 1700, 1800, 1900
        return False
    elif annee % 4 == 0: # 2020, 2024, 2028
        return True
    return False # 2021, 2022, 2023

def temps_avant(date):
    ...

class Accueil(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Anniversaires")
        ...
        self.texte_anniv = QLabel("Patientez...")
        self.texte_meteo = QLabel("")

        bouton_donnees = QPushButton("voir les données")
        bouton_donnees.clicked.connect(self.afficher_modifier)
        self.bouton_meteo = QPushButton("...")
        bouton_nouveautes = QPushButton("nouveautés")
        bouton_nouveautes.clicked.connect(self.afficher_nouveau)
        bouton_fermer = QPushButton("fermer")
        bouton_fermer.clicked.connect(self.close)

        boite_actions = QHBoxLayout(self)
        boite_actions.addWidget(bouton_donnees)
        boite_actions.addWidget(self.bouton_meteo)
        boite_actions.addWidget(bouton_nouveautes)
        boite_actions.addWidget(bouton_fermer)

        boite = QVBoxLayout()
        boite.addWidget(self.texte_anniv)
        boite.addWidget(self.texte_meteo)
        boite.addLayout(boite_actions)

        contenu = QWidget()
        contenu.setLayout(boite)
        self.setCentralWidget(contenu)

        meteo = threading.Thread(target=self.actu_meteo)
        meteo.start()
        ...
        self.actu_affichage()
        self.timer_actualiser = QTimer(self)
        self.timer_actualiser.timeout.connect(self.actu_affichage)
        self.timer_actualiser.start(1000)

        self.timer_fermeture_auto = QTimer(self)
        self.timer_fermeture_auto.setSingleShot(True)
        self.timer_fermeture_auto.timeout.connect(self.fermer_auto)
        self.timer_fermeture_auto.start(20*1000)

    def actu_affichage(self):
        self.actu_contenu()

        message = ""
        police = self.texte_anniv.font()

        aujourdhui = []
        for personne in self.personnes:
            if strftime("%d/%m") == "/".join(personne[0].split("/")[0:2]): # Anniversaire
                aujourdhui.append(["l'anniversaire", personne[1]+f"\n(âge : {personne[6]})"])
        ...
        self.texte_anniv.setFont(police)
        self.texte_anniv.setText(message)
    ...
    def actu_meteo(self):
        try:
            sock = rq.get(f"https://prevision-meteo.ch/services/json/{ville}")
            self.meteo = sock.json()
            sock.close()

            self.texte_meteo.setText(f"Température à {ville.capitalize()} : {self.meteo['current_condition']['tmp']}°C\nCondition : {self.meteo['current_condition']['condition']}")

            self.bouton_meteo.setText("météo")
            self.bouton_meteo.clicked.connect(self.afficher_meteo)

            log("météo actualisée")
        except (ConnectionError, rq.exceptions.JSONDecodeError):
            self.texte_meteo.setText("Erreur lors de la récupération de la météo.")
            self.bouton_meteo.setText("reconnecter à internet")
            self.bouton_meteo.clicked.connect(self.actu_meteo)
            log("!!! problème de connection !!!")

    def afficher_modifier(self):
        self.action = True
        self.setWindowFlags(self.windowFlags() & ~Qt.WindowStaysOnTopHint)
        self.show()
        self.fenetre_modifier = Modifier(self)
        self.fenetre_modifier.show()

    def afficher_meteo(self):
        self.action = True
        self.setWindowFlags(self.windowFlags() & ~Qt.WindowStaysOnTopHint)
        self.show()
        self.fenetre_meteo = Meteo(self)
        self.fenetre_meteo.show()

    def animation(self, nombre=20, etape=True):
        if nombre:
            palette = self.texte_anniv.palette()
            palette.setColor(self.texte_anniv.foregroundRole(), QColor(("red" if etape else "black")))
            self.texte_anniv.setPalette(palette)
            timer = QTimer(self)
            timer.setSingleShot(True)
            timer.timeout.connect(lambda: self.animation(nombre-1, not etape))
            timer.start(500)

    def fermer_auto(self):
        if not self.action:
            log("fermeture automatique lancée")
            self.pourcentage_restant = 1.0
            self.timer_fermer = QTimer(self)
            self.timer_fermer.timeout.connect(self.fermer_degrade)
            self.timer_fermer.start(10)


    def fermer_degrade(self):
        if self.pourcentage_restant > 0.01:
            self.setWindowOpacity(self.pourcentage_restant)
            self.pourcentage_restant -= 0.01
        else:
            self.timer_fermer.stop()
            self.close()

    def afficher_nouveau(self):
        self.action = True
        boite_info = QMessageBox(self)
        boite_info.setWindowTitle("Nouveautés")
        boite_info.setText("""Nouveau (le 24/06/2025) :
- refonte graphique complète de l'interface
- recodage entier de l'application en vue de nouvelles mises à jour""")
        boite_info.setStandardButtons(QMessageBox.Ok)
        boite_info.setIcon(QMessageBox.Information)
        boite_info.exec()

    def closeEvent(self, event):
        log("fermeture de la fenêtre")
        self.timer_actualiser.stop()
        self.timer_fermeture_auto.stop()
        try:
            self.fenetre_meteo.close()
        except AttributeError:
            pass
        try:
            self.fenetre_modifier.close()
        except AttributeError:
            pass
        event.accept()
...
class Modifier(QWidget):
    def __init__(self, fenetre_principale):
        ...
        bouton_rechercher = QPushButton("Recherche une fête")
        bouton_rechercher.clicked.connect(self.chercher_fete)
        bouton_modifier = QPushButton("Modifier")
        bouton_modifier.clicked.connect(self.modifier)
        bouton_creer = QPushButton("Nouvelle ligne")
        bouton_creer.clicked.connect(self.ajouter_ligne)
        bouton_supprimer = QPushButton("Supprimer ligne")
        bouton_supprimer.clicked.connect(self.supprimer_ligne)

        boite_actions = QHBoxLayout()
        boite_actions.addWidget(bouton_rechercher)
        boite_actions.addWidget(bouton_modifier)
        boite_actions.addWidget(bouton_creer)
        boite_actions.addWidget(bouton_supprimer)

        titres = ['Date', 'Prénom', 'Âge', 'Saint', 'Anniversaire dans', 'Fête dans']
        self.tableau = QTableWidget()
        self.tableau.setColumnCount(len(titres))
        self.tableau.setHorizontalHeaderLabels(titres)
        self.tableau.setEditTriggers(QAbstractItemView.NoEditTriggers)
        self.tableau.setSelectionMode(QAbstractItemView.SingleSelection)
        self.tableau.doubleClicked = lambda e: self.modifier() if e.button() == Qt.LeftButton else 0
        self.tableau.mouseReleaseEvent = lambda e: self.clic(e)

        self.boite = QVBoxLayout()
        self.boite.addLayout(boite_actions)
        self.boite.addWidget(self.tableau)
        self.setLayout(self.boite)

        self.afficher_donnees()

    def afficher_donnees(self):
        self.tableau.setRowCount(len(self.fenetre_principale.personnes))

        self.fenetre_principale.actu_contenu()
        for ligne, personne in enumerate(self.fenetre_principale.personnes):
            for colonne, donnee in enumerate(personne[0:2]+[personne[6]]+personne[2:5]):
                self.tableau.setItem(ligne, colonne, QTableWidgetItem(str(donnee)))

    def chercher_fete(self):
        prenom, choix = QInputDialog.getText(None, "Rechercher une fête", "Entrez le prénom à rechercher :")
        if not choix:
            return
        try:
            QMessageBox.information(None, "Fête", f"La fête de {prenom} est le {' et '.join(saint[prenom])}")
        except KeyError:
            QMessageBox.warning(None, "Fête", f"Prénom {prenom} inconnu de la base de donnée.")

    def clic(self, e):
        if e.button() != Qt.RightButton:
            return
        nb = self.tableau.currentRow()
        menu = QMenu(self)
        modifier = menu.addAction("Modifier")
        supprimer = menu.addAction("Supprimer")
        menu.addSeparator()
        prenom = menu.addAction(f"Prénom : {self.fenetre_principale.personnes[nb][5]}")
    ...
    def modifier(self, nb=None):
        if not nb:
            nb = self.tableau.currentRow()
        self.fenetre_modifieur = Modifieur(self, nb)
        self.fenetre_modifieur.exec()

    def supprimer_ligne(self):
        nb = self.tableau.currentRow()
        question = QMessageBox()
        question.setWindowTitle("Confirmation")
        question.setText(f"Supprimer {self.fenetre_principale.personnes[nb][1]} ?")
        question.setIcon(QMessageBox.Question)
        question.setStandardButtons(QMessageBox.Yes | QMessageBox.Cancel)
        retour = question.exec()
        if retour == QMessageBox.Yes:
            del self.fenetre_principale.personnes[nb]
            self.fenetre_principale.enregistrer()
            self.afficher_donnees()

    def ajouter_ligne(self):
        self.fenetre_principale.personnes.append(["xx/xx", "Nouveau", "inconnu", "xxx", "xxx", "Nouveau"])
        self.fenetre_principale.enregistrer()
        self.modifier(-1)

class Modifieur(QDialog):
    def __init__(self, fenetre_modifier, nb):
        super().__init__()
        self.setWindowTitle("Modifier")
        self.fenetre_modifier = fenetre_modifier
        self.nb = nb

        self.boite = QGridLayout()
        champs = [["Date de naissance", self.fenetre_modifier.fenetre_principale.personnes[nb][0]],
                 ["Nom usuel", self.fenetre_modifier.fenetre_principale.personnes[nb][1]],
                 ["Prénom", self.fenetre_modifier.fenetre_principale.personnes[nb][5]]]
        self.entrees = []
        for index, texte in enumerate(champs):
            self.boite.addWidget(QLabel(texte[0]), index, 0)
            entree = QLineEdit()
            entree.setText(texte[1])
            self.boite.addWidget(entree, index, 1)
            self.entrees.append(entree)
        bouton_valider = QPushButton("valider")
        bouton_valider.clicked.connect(self.valider)
        bouton_cara_spec = QPushButton("caractères spéciaux")
        bouton_cara_spec.clicked.connect(self.charmap)
        self.boite.addWidget(bouton_valider, index+1, 0)
        self.boite.addWidget(bouton_cara_spec, index+1, 1)

        self.setLayout(self.boite)

    def valider(self):
        self.fenetre_modifier.fenetre_principale.personnes[self.nb][0] = self.entrees[0].text()
        self.fenetre_modifier.fenetre_principale.personnes[self.nb][1] = self.entrees[1].text()
        self.fenetre_modifier.fenetre_principale.personnes[self.nb][5] = self.entrees[2].text()
        self.fenetre_modifier.fenetre_principale.enregistrer()
        self.fenetre_modifier.afficher_donnees()
        self.close()
...
class MeteoImageWorker(QObject):
    ...
    def run(self):
        liste_images = []
        for j in range(5):
            jour = self.donnees[f"fcst_day_{j}"]
            image = Image.open(rq.get(jour["icon"], stream=True).raw)
            image = image.resize((20, 20), Image.LANCZOS)
            data = np.array(image)
            h, w, ch = data.shape
            bytes_per_line = ch * w
            qimage = QImage(data.data, w, h, bytes_per_line, QImage.Format_RGBA8888)
            pixmap = QPixmap.fromImage(qimage)
            liste_images.append(pixmap)
        self.finished.emit(liste_images)
...
class Meteo(QWidget):
    def __init__(self, fenetre_principale):
        ...
        self.setWindowIcon(QIcon(ICONE_METEO))

        self.fenetre_principale = fenetre_principale
        meteo = threading.Thread(target=self.fenetre_principale.actu_meteo)
        meteo.start()

        self.progression = QProgressBar(self)
        self.progression.setRange(0, 5)

        self.tableau = QGridLayout()

        self.texte_source = QLabel("Source météo : https://prevision-meteo.ch/")
        self.texte_source.mousePressEvent = lambda e: self.ouvrir_source()

        self.boite = QVBoxLayout()
        self.boite.addWidget(self.progression)
        self.boite.addLayout(self.tableau)
        self.boite.addWidget(self.texte_source)
        self.setLayout(self.boite)

        worker_images = MeteoImageWorker(self.fenetre_principale.meteo)
        worker_images.finished.connect(self.charger_donnees)
        worker_images.run()

    def charger_donnees(self, images):
        log("chargement données météo")
        for i in range(24):
            self.tableau.addWidget(QLabel(f"{i}h"), 0, i+1)

        for j in range(5):
            self.progression.setValue(j)
            jour = self.fenetre_principale.meteo[f"fcst_day_{j}"]
            ...
            label_image = QLabel(n)
            label_image.setPixmap(images[j])
            self.tableau.addWidget(label_image, j+1, 0)
            texte_jour = QLabel(n)
            self.tableau.addWidget(texte_jour, j+1, 1)

            for i, donnees in enumerate(j for i, j in jour["hourly_data"].items()):
                self.tableau.addWidget(QLabel(f"{donnees['TMP2m']}°C"), j+1, i+2)
...
#            self.t.insert(j, "end", text="vitesse du vent", values=list(f"{jour['hourly_data'][d]['WNDSPD10m']} km/h" for d in jour["hourly_data"]))
    ...
    def ouvrir_source(self):
        webbrowser.open("https://prevision-meteo.ch/")
